...
type IconName = "arrow" | "back" | "chart" | "check" | "clock" | "file" | "invoice" | "message" | "people" | "phone" | "plus" | "search" | "spark" | "target";
...
const services = [
  {
    ...
    description: "Tez, makale veya araştırmadaki veriler analiz edilir; sonuçlar akademik standartlara uygun şekilde raporlanır.",
  ...
  },
  {
    ...
    description: "Araştırma için gerekli örneklem büyüklüğü veya mevcut çalışmanın istatistiksel gücü hesaplanır.",
  ...
  },
  {
    ...
    description: "Çalışmanın yöntemini ve temel bulgularını dergi standartlarına uygun tek bir görsel anlatıma dönüştürün.",
  ...
  },
  {
    ...
    description: "Kurum, proje veya bütçe onayı süreçleri için hizmet kapsamını ve ücret bilgisini içeren proforma fatura talep edin.",
  ...
  },
  {
    ...
    description: "Akademik çalışmalara yönelik mobil uygulama ihtiyacını paylaşın; kapsam ve geliştirme planı birlikte oluşturulsun.",
    ...
    <AdminShell>
      <main className="request-page">
        ...
        {step === 1 && <><RequestHero step={1} title="Nasıl yardımcı olabiliriz?" description="İhtiyaca en uygun hizmeti seçin. Sonraki adımda çalışmanın ayrıntılarını birlikte netleştireceğiz." />
          ...
          <section className="guidance-panel"><span className="guidance-icon"><Icon name="message" size={23} /></span><div><p className="eyebrow light">KARAR VEREMEDİNİZ Mİ?</p><h2>Çalışmayı kısaca anlatın, doğru hizmeti birlikte belirleyelim.</h2><p>Uzman ekibimiz talebi inceleyip uygun hizmete yönlendirsin.</p></div><button>Yönlendirme iste <Icon name="arrow" size={16} /></button></section>
          <ActionBar service={selectedService} label="Çalışma bilgilerine devam et" disabled={!selectedService} onNext={() => goTo(2)} /></>}

        {step === 2 && <><RequestHero step={2} title="Çalışmayı tanıyalım" description="Uzmanımızın kapsamı doğru değerlendirebilmesi için çalışmayla ilgili temel bilgileri paylaşın." />
          <div className="request-form-layout"><section className="request-form-panel"><div className="selected-service-line"><span className="service-icon small"><Icon name={selectedService?.icon ?? "chart"} size={18} /></span><div><small>SEÇİLEN HİZMET</small><strong>{selectedService?.title}</strong></div><button onClick={() => goTo(1)}>Değiştir</button></div>
            <div className="form-field"><label>Çalışmanın kısa adı *</label><input value={title} onChange={e => setTitle(e.target.value)} placeholder="Örn. Hemşirelerde tükenmişlik düzeyi araştırması" /></div>
            ...
            <div className="form-field"><label>Çalışmanın konusu ve amacı *</label><textarea value={purpose} onChange={e => setPurpose(e.target.value)} placeholder="Araştırma sorusunu, amacını ve analizden beklentiyi kısaca açıklayın." /></div>
            <div className="form-field"><label>Eklemek istediğiniz notlar</label><textarea className="small" value={notes} onChange={e => setNotes(e.target.value)} placeholder="Uzmanımızın bilmesi gereken ek bir detay varsa yazılabilir." /></div>
        ...
        {step === 3 && <><RequestHero step={3} title="Dosyaları ekleyin" description="Analizde kullanılacak veri ve dokümanları yükleyin. Daha sonra Sipariş Kalemleri bölümünden yeni dosya eklenebilir." />
          <div className="request-form-layout"><section className="request-form-panel file-step"><label className="request-upload clickable-upload" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); addFiles(Array.from(event.dataTransfer.files)); }}><Icon name="file" size={26} /><h2>Dosyaları buraya bırakın</h2><p>Office, SPSS, PDF, görsel, video veya arşiv · Dosya başına en fazla 25 MB</p><input type="file" multiple accept=".doc,.docx,.rtf,.xls,.xlsx,.csv,.ppt,.pptx,.pps,.ppsx,.sav,.zsav,.sps,.spv,.spo,.pdf,.jpg,.jpeg,.png,.gif,.webp,.svg,.mp4,.mov,.avi,.webm,.zip,.rar,.7z,.tar,.gz" onChange={(event) => { addFiles(Array.from(event.target.files ?? [])); event.target.value = ""; }} /><span className="request-upload-action">{files.length ? "Başka dosya ekle" : "Dosya seç"}</span></label>
        ...
        {step === 4 && <><RequestHero step={4} title="Teslimat tercihlerini belirleyin" description="Analiz sonuçlarını hangi kapsamda ve ne kadar sürede teslim almak istediğinizi seçin." />
        ...
        {step === 5 && !submitted && <><RequestHero step={5} title="Talebi gözden geçirin" description="Göndermeden önce hizmet, çalışma ve teslimat bilgilerini son kez kontrol edin." />
        ...
        {step === 5 && submitted && <section className="request-success"><span><Icon name="check" size={28} /></span><p className="eyebrow">TALEBİ ALINDI</p><h1>Uzman değerlendirmesi başladı</h1><p>Talep incelendikten sonra kapsam, ücret ve teslim tarihi belirlenecek.</p><div><strong>Geçici talep numarası</strong><code>TA260724018</code></div><Link href="/admin/siparisler">Siparişlere git <Icon name="arrow" size={16} /></Link></section>}
      </main>
    </AdminShell>
  );
}
...
function RequestHero({ step, title, description }: { step: number; title: string; description: string }) {
  return <header className="request-hero"><div><p className="eyebrow">YENİ TALEP · {step}. ADIM</p><h1>{title}</h1><p>{description}</p></div><div className="request-note"><Icon name="clock" size={20} /><div><strong>Bilgiler korunuyor</strong><span>Önceki adımlara dönebilirsiniz</span></div></div></header>;
}
