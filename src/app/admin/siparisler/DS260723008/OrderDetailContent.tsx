        ...
        <header className="detail-hero admin-detail-hero">
          <div className="admin-hero-left">
            <div className="detail-title">
              ...
              <div>
                <div className="detail-title-topline">
                  <Link className="back-link detail-hero-back" href={backHref}><Icon name="back" size={14} />{audience === "analizor" ? "İşlerime dön" : "Siparişlere dön"}</Link>
                  <p className="eyebrow">{orderCode} · 04.08.2026 10:36</p>
                </div>
                <h1>{orderTitle}</h1>
              </div>
            </div>
  ...
  return (
    <section className="detail-panel messages-panel panel-direct" aria-label="Sipariş yazışmaları">
      <div className="message-composer">
        ...
        <div><MessageTemplatePicker onSelect={setMessage} /><button className="send" type="button" onClick={send} disabled={!message.trim()}>Gönder <Icon name="arrow" size={15} /></button></div>
      </div>
...
function AnalysisResultsSection() {
  ...
  function fmtSize(b: number) { return b < 1024*1024 ? `${Math.max(1,Math.round(b/1024))} KB` : `${(b/(1024*1024)).toLocaleString("tr-TR",{maximumFractionDigits:1})} MB`; }
  function makeHandler(setter: React.Dispatch<React.SetStateAction<{ name: string; size: string }[]>>) {
    ...
    <section className="detail-panel panel-direct" aria-label="Teslimat raporları">
      ...
      <div className="result-subsection">
        <div className="result-subsection-head">
          ...
          <input ref={deliveryRef} className="visually-hidden-file-input" type="file" multiple onChange={makeHandler(setDeliveryFiles)} />
      ...
      <div className="result-subsection">
        <div className="result-subsection-head">
          ...
          <input ref={reportRef} className="visually-hidden-file-input" type="file" multiple onChange={makeHandler(setReportFiles)} />
