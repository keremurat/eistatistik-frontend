...
// ── İkon ─────────────────────────────────────────────────────────────────────
type IName = "folder" | "plus" | "x" | "dots" | "trash" | "edit" | "archive" | "check";
...
// ── Kart 3-nokta menüsü ───────────────────────────────────────────────────────
function CardMenu({ onEdit, onArchive, onDelete }: {
  onEdit: () => void;
  onArchive: () => void;
  onDelete: () => void;
}) {
  ...
  const itemBase: React.CSSProperties = {
    width: "100%", padding: ".44rem .65rem", border: 0, borderRadius: 6,
  ...
  const menu = (
    <div ref={panRef} role="menu" style={{
      position: "fixed", top: pos.top, left: pos.left, width: 156, zIndex: 9999,
      ...
      <button role="menuitem" style={{ ...itemBase, color: "#3f556a" }}
      ...
      <button role="menuitem" style={{ ...itemBase, color: "#3f556a" }}
        ...
        <Icon name="archive" size={14} />Arşivle
      </button>
      <div style={{ height: 1, background: "var(--line)", margin: ".25rem 0" }} />
      <button role="menuitem" style={{ ...itemBase, color: "#c85a51" }}
...
// ── Görev kartı ───────────────────────────────────────────────────────────────
function GorevCard({ gorev, onEdit, onArchive, onDelete }: {
  gorev: Gorev;
  onEdit: () => void;
  onArchive: () => void;
  onDelete: () => void;
  ...
  return (
    <article className="gorev-card">
      ...
      <div className="gorev-card-foot">
        ...
        <CardMenu onEdit={onEdit} onArchive={onArchive} onDelete={onDelete} />
      </div>
    </article>
  );
}
...
function GorevModal({ panolar, initial, onClose, onSave }: {
  ...
  function parseTarihSaat(tarih: string) {
    const parts = tarih.split(" ");
    return { tarih: parts[0] ?? "", saat: parts[1] ?? "" };
  }

  const init = initial ? parseTarihSaat(initial.tarih) : { tarih: "", saat: "" };
  ...
  function handleSave() {
    ...
    onSave({
      ...
      ...(isEdit ? { siparisKodu: initial!.siparisKodu, musteriAdi: initial!.musteriAdi, musteriTel: initial!.musteriTel } : {}),
    });
    setSaved(true);
      ...
      <section className="admin-modal" role="dialog" aria-modal="true" aria-labelledby="gm-title">
        ...
        <footer className="admin-modal-foot">
          ...
          <button className="btn-save" onClick={handleSave} disabled={!baslik.trim()}
            ...
            {saved
              ? <><Icon name="check" size={14} /> {isEdit ? "Güncellendi" : "Kaydedildi"}</>
...
export default function GorevListesiKartPage() {
  ...
  function updateGorev(g: Omit<Gorev, "id"> & { id?: number }) {
    setGorevler(prev => prev.map(existing =>
      existing.id === g.id ? { ...existing, ...g } as Gorev : existing
    ));
  }

  function archiveGorev(id: number) {
  ...
  return (
    <AdminShell>
              ...
              <div key={pano.key} className="kanban-col">
                ...
                {cards.map(gorev => (
                  <GorevCard
                    key={gorev.id}
                    gorev={gorev}
                    onEdit={() => setEditingGorev(gorev)}
                    onArchive={() => archiveGorev(gorev.id)}
                    onDelete={() => deleteGorev(gorev.id)}
                  />
                ))}
              </div>
            );
      ...
      {editingGorev && (
        <GorevModal
          panolar={panolar}
          initial={editingGorev}
          onClose={() => setEditingGorev(null)}
