    ...
    <div className="app-shell">
      ...
      <main className="request-page">
        ...
        {step === 3 && <><RequestHero step={3} title="Dosyalarınızı ekleyin" description="Analizde kullanılacak veri ve dokümanları yükleyin. Daha sonra Sipariş Kalemleri bölümünden yeni dosya ekleyebilirsiniz." />
          <div className="request-form-layout"><section className="request-form-panel file-step"><label className="request-upload clickable-upload" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); addFiles(Array.from(event.dataTransfer.files)); }}><Icon name="file" size={26} /><h2>Dosyalarınızı buraya bırakın</h2><p>Office, SPSS, PDF, görsel, video veya arşiv · Dosya başına en fazla 25 MB</p><input type="file" multiple accept=".doc,.docx,.rtf,.xls,.xlsx,.csv,.ppt,.pptx,.pps,.ppsx,.sav,.zsav,.sps,.spv,.spo,.pdf,.jpg,.jpeg,.png,.gif,.webp,.svg,.mp4,.mov,.avi,.webm,.zip,.rar,.7z,.tar,.gz" onChange={(event) => { addFiles(Array.from(event.target.files ?? [])); event.target.value = ""; }} /><span className="request-upload-action">{files.length ? "Başka dosya ekle" : "Dosya seç"}</span></label>
