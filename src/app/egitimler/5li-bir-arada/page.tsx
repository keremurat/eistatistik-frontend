...
function EducationPayment({ method, onMethod }: { method: "transfer" | "card"; onMethod: (value: "transfer" | "card") => void }) {
  ...
  return <div className="payment-layout education-payment"><section className="detail-panel payment-main">
    <div className="payment-method-switch">
      <div className="payment-tabs" role="group" aria-label="Ödeme yöntemi">
        <button type="button" className={method === "transfer" ? "active" : ""} onClick={() => onMethod("transfer")} aria-pressed={method === "transfer"}><EducationIcon name="file" size={18} /><span>Havale / EFT</span><small>Banka transferi ve manuel onay</small></button>
        <button type="button" className={method === "card" ? "active" : ""} onClick={() => onMethod("card")} aria-pressed={method === "card"}><EducationIcon name="card" size={18} /><span>Kredi kartı</span><small>Akbank 3D Secure</small></button>
      </div>
    </div>
    ...
    {method === "transfer" ? <div className="bank-box"><h3>Banka bilgileri</h3><dl><div><dt>Adı Soyadı</dt><dd>Naci MURAT</dd></div><div><dt>Banka</dt><dd>Akbank (0046) — 19 Mayıs Üniversitesi Şubesi (01389)</dd></div><div><dt>Hesap No</dt><dd><code>0014316</code></dd></div><div><dt>IBAN</dt><dd><code>TR64 0004 6013 8988 8000 0143 16</code><button>Kopyala</button></dd></div><div><dt>Açıklama</dt><dd><code>TR260723010</code><button>Kopyala</button></dd></div></dl><input ref={receiptInputRef} className="visually-hidden-file-input" type="file" accept=".pdf,.jpg,.jpeg,.png" onChange={(event) => { setReceiptName(event.target.files?.[0]?.name ?? ""); event.target.value = ""; }} /><div className="receipt-upload clickable-upload" role="button" tabIndex={0} aria-label="Dekont dosyası seç" onClick={() => receiptInputRef.current?.click()} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); receiptInputRef.current?.click(); } }}><EducationIcon name="file" size={22} /><div><strong>{receiptName || "Dekont yükle"}</strong><span>{receiptName ? "Dosya seçildi · Değiştirmek için alana tıklayın" : "PDF, JPG veya PNG · En fazla 10 MB"}</span></div><button type="button" tabIndex={-1}>{receiptName ? "Değiştir" : "Dosya seç"}</button></div></div>
    : <div className="secure-card-redirect"><span><EducationIcon name="card" size={25} /></span><div><p className="eyebrow">AKBANK 3D SECURE</p><h3>Ödemeniz Akbank güvencesiyle tamamlanacak</h3><p>Sonraki ekranda Akbank Ortak Ödeme sayfasına yönlendirileceksiniz. Kart bilgileriniz eistatistik tarafından alınmaz veya kaydedilmez.</p></div></div>}
